...
///--  pfAllCandidateToken_     =  consumes<reco::PFCandidateCollection>   (iConfig.getParameter<InputTag>("pfAllCandidate")); 
  ...
  // for boosted taus
  dumpBoostedTaus_           = iConfig.getParameter<bool>("dumpBoostedTaus");
  boostedTauCollectionNoOverLap_    = consumes<vector<pat::Tau> >(iConfig.getParameter<InputTag>("cleanedBoostedTauSrc"));
  boostedTauCollection_             = consumes<vector<pat::Tau> >(iConfig.getParameter<InputTag>("boostedTauSrc"));
  ...
  if (dumpBoostedTaus_) branchesBoostedTaus(tree_); // for boosted taus
...
phoJetNtuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  if (dumpBoostedTaus_)  fillBoostedTaus(iEvent); /// for boosted taus
